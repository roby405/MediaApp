...
function AudioCard({ file, viewMode, onClick }: AudioCardProps) {
      ...
      {viewMode === "grid" && (
        ...
        >
          ...
          <div className="pointer-events-none absolute left-0 bottom-0 right-0 p-3">
            <div className="truncate text-md drop-shadow-sm">
              {file.metadata.title}
            </div>
            <div className="text-gray-400">{file.metadata.artist}</div>
          </div>
        </div>
      )}
      {viewMode === "line" && (
          ...
          <div className="flex flex-row w-full h-full pointer-events-none">
            ...
            <div className="flex-1 min-w-0 pt-1 pl-1 pr-2 h-full flex flex-col">
              <div className="flex flex-row relative">
                <div className="truncate text-lg flex-1">
                  {file.metadata.title}
                </div>
                <MediaMenuButton file={file} type="line" />
              </div>
              <div className="flex-1 pb-2 pr-8 flex flex-row justify-between text-xs items-end">
                <div className="truncate text-left">{file.metadata.artist}</div>
                <div className="truncate text-right">
                  {formatLength(file.metadata.duration)}
                </div>
              </div>
            </div>
          </div>
