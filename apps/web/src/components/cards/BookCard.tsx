...
function BookCard({ file, viewMode, onClick }: BookCardProps) {
  ...
  return (
    <>
      {viewMode === "grid" && (
        <div
          onClick={onClick}
          className="relative shadow-md group w-full aspect-2/3 rounded-xl bg-secondary select-none"
        >
          {imageUrl ? (
        ...
        <MediaMenuButton file={file} type="grid" />
          ...
          <div className="pointer-events-none absolute left-0 bottom-0 right-0 p-3">
            <div className="truncate text-md drop-shadow-sm">{file.metadata.title}</div>
          ...
          <div className="flex flex-row w-full h-full pointer-events-none">
            ...
            <div className="flex-1 min-w-0 pt-1 pl-1 pr-2 h-full flex flex-col">
              <div className="flex flex-row relative">
                <div className="truncate text-lg flex-1">{file.metadata.title}</div>
