...
function NavButton({ label, icon: Icon, isActive, onClick }: NavButtonProps) {
  return (
    ...
    >
      ...
      <span className={`text-md ${isActive ? "font-bold" : ""}`}>{label}</span>
    </button>
  );
}
  ...
  return (
    <>
      ...
      >
        ...
        {isActive && (
          <div className="absolute bottom-full mb-10 bg-secondary flex flex-col w-40 h-26 gap-2 rounded-2xl text-lg">
            <button
              className="flex-1"
...
function MobileBottomBar() {
  ...
  const setDeferred = useNavStore((state) => state.setDeferred);
  return (
    <div className="h-20 px-4 py-2 mt-4 rounded-t-xl flex flex-row gap-2 bg-secondary text-text">
      {NAV_ITEMS.map((item) =>
        item.id !== "import" ? (
          <NavButton
            key={item.id}
            ...
            onClick={() => {setDeferred(false); setScreen(item.id);}}
          />
        ) : (
          <ImportButton
            key={item.id}
            {...item}
            isActive={activeScreen === item.id}
            onClick={() => {setDeferred(true); setScreen(item.id);}}
          />
        ),
      )}
    </div>
